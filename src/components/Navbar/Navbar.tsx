...
export default function Navbar({ cartCount = 0, activePath, onSearchSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    const handler: EventListener = (ev) => {
      ...
      if (e.shiftKey && document.activeElement === first) { e.preventDefault(); (last as HTMLElement | undefined)?.focus(); }
      else if (!e.shiftKey && document.activeElement === last) { e.preventDefault(); (first as HTMLElement | undefined)?.focus(); }
    };
    ...
    >
      <div className="mi-drawer__panel mi-panel" role="dialog" aria-modal="true" aria-label="Menu" ref={panelRef}>
        ...
        <form role="search" onSubmit={submitSearch} style={{ marginBottom: 10 }}>
          <div className="mi-panel" style={{ display: "flex", alignItems: "center", gap: 8, height: 44, padding: "0 10px" }}>
            <SearchIcon />
            <input
              ...
              style={{ background: "transparent", border: "none", color: "var(--text)", padding: "0 8px", outline: "none", width: "100%" }}
            />
          </div>
        </form>
      ...
      <div className="mi-container">
        <div className="mi-nav__bar">
          ...
          <div className="mi-actions">
            {/* Search (desktop) */}
            <form className="mi-search mi-desktop" role="search" aria-label="Site search" onSubmit={submitSearch}>
            ...
            <button className="mi-iconbtn mi-mobile" aria-label="Open search" onClick={openSearchInDrawer}>
              <SearchIcon />
            </button>
            <button
    ...
    <svg width="34" height="34" viewBox="0 0 32 32" aria-hidden="true">
      ...
      <defs>
        <linearGradient id="g-mi" x1="0" y1="0" x2="0" y2="32">
          <stop stopColor="#1b232b"/><stop offset="1" stopColor="#12181f"/>
        </linearGradient>
      </defs>
    </svg>
  );
...
function CartIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text)" strokeWidth="2" aria-hidden="true">
