...
const WishlistProductImage: React.FC<WishlistProductImageProps> = memo(({ 
  ...
  const getOptimizedImageSrc = (): string | null => {
    ...
    if (product?.gallery && product.gallery.length > 0) {
      ...
      if (galleryImage.includes('digitaloceanspaces.com') || galleryImage.includes(import.meta.env.VITE_CDN_BASE || '')) {
        return galleryImage;
      }
    ...
    if (item.productImageUrl) {
      ...
      if (item.productImageUrl.includes('digitaloceanspaces.com') || item.productImageUrl.includes(import.meta.env.VITE_CDN_BASE || '')) {
