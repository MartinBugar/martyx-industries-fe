...
import { type TabContent, type PrintInfoData } from '../../lib/types/product';
import styles from './ProductTabs.module.css';

// PrintInfo tab content interface - matches the union type in product types
...
const PrintInfoTab: React.FC<PrintInfoTabProps> = ({ content }) => {
  ...
  if (content.kind !== 'printInfo') {
    return (
      <div className={styles['print-info-unavailable']}>
  ...
  return (
    <div className={styles['print-info-container']}>
      {/* Print Settings Section */}
      <section className={styles['print-settings-section']}>
        ...
        <div className={styles['settings-grid']}>
          <div className={styles['setting-item']}>
            ...
            <span className={styles['setting-value']}>{printSettings.printTime}</span>
          </div>
          ...
          <div className={styles['setting-item']}>
            ...
            <span className={styles['setting-value']}>{printSettings.layerHeight}</span>
          </div>
          ...
          <div className={styles['setting-item']}>
            ...
            <span className={styles['setting-value']}>{printSettings.infill}</span>
          </div>
          ...
          <div className={styles['setting-item']}>
            ...
            <span className={`${styles['setting-value']} ${printSettings.supports ? styles['required'] : styles['not-required']}`}>
          ...
          {printSettings.estimatedCost && (
            <div className={styles['setting-item']}>
              ...
              <span className={`${styles['setting-value']} ${styles['cost']}`}>{printSettings.estimatedCost}</span>
            </div>
          )}
        </div>
        ...
        <div className={styles['materials-section']}>
          <h4 className={styles['subsection-title']}>{t('printInfo.materials', 'Recommended Materials')}</h4>
          <div className={styles['materials-list']}>
            {printSettings.materials.map((material, index) => (
              <span key={index} className={styles['material-tag']}>
                {material}
              </span>
            ))}
          </div>
      ...
      <section className={styles['rc-components-section']}>
        <div className={styles['section-title']}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className={styles['components-list']}>
          {rcComponents.map((component, index) => (
            <div key={index} className={`${styles['component-item']} ${component.required ? styles['required'] : styles['optional']}`}>
              <div className={styles['component-header']}>
                <div className={styles['component-name']}>
                  {component.required ? (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={styles['required-icon']}>
                      <circle cx="12" cy="12" r="10"></circle>
                      <line x1="12" y1="8" x2="12" y2="12"></line>
                      <line x1="12" y1="16" x2="12.01" y2="16"></line>
                    </svg>
                  ) : (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={styles['optional-icon']}>
                      <circle cx="12" cy="12" r="10"></circle>
                      <line x1="12" y1="8" x2="12" y2="12"></line>
                      <line x1="12" y1="16" x2="12.01" y2="16"></line>
                    </svg>
                  ...
                  <span className={styles['quantity']}>×{component.quantity}</span>
                </div>
                {component.estimatedPrice && (
                  <span className={styles['component-price']}>{component.estimatedPrice}</span>
                )}
              </div>
        ...
        {/* Component Summary */}
        <div className={styles['components-summary']}>
          <div className={styles['summary-item']}>
            <span className={styles['summary-label']}>{t('printInfo.requiredComponents', 'Required Components')}:</span>
            <span className={styles['summary-value']}>{rcComponents.filter(c => c.required).length}</span>
          </div>
          <div className={styles['summary-item']}>
            <span className={styles['summary-label']}>{t('printInfo.optionalComponents', 'Optional Components')}:</span>
            <span className={styles['summary-value']}>{rcComponents.filter(c => !c.required).length}</span>
          </div>
        </div>
      </section>
      ...
      {additionalNotes && additionalNotes.length > 0 && (
        <section className={styles['additional-notes-section']}>
          <div className={styles['section-title']}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <ul className={styles['notes-list']}>
            {additionalNotes.map((note, index) => (
              <li key={index} className={styles['note-item']}>{note}</li>
            ))}
          </ul>
        </section>
      )}
