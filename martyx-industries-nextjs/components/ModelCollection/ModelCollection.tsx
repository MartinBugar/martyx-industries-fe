...
const ModelCollection: React.FC = () => {
  ...
  const loadModelStatus = async (productId: string, orderId: string): Promise<{ is_completed: boolean; is_public: boolean }> => {
    try {
      ...
      const url = `${getApiBaseUrl()}/api/user-models/${productId}/status?order_id=${orderId}`;
  ...
  const loadPhotosForModel = async (productId: string): Promise<ModelPhoto[]> => {
    try {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/api/user-photos/${productId}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const deletePhoto = async (photoId: number, modelProductId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/api/user-photos/${photoId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok) {
        if (response.status === 500) {
          ...
          if (process.env.NEXT_PUBLIC_MOCK_DELETES === 'true') {
  ...
  const updateModelStatus = async (productId: string, orderId: string, field: 'is_completed' | 'is_public', value: boolean) => {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/api/user-models/${productId}/status`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok) {
        if (response.status === 500) {
          ...
          if (process.env.NEXT_PUBLIC_MOCK_UPDATES === 'true') {
